<script lang="ts">
    ...
    const otherViewers = $derived(
        presence
            .getViewersForReport(data.report.id)
            .filter((u) => u.userId !== data.currentUserId),
    );
...
<div class="space-y-6">
    ...
    <Button
        variant="ghost"
        class="gap-1.5 pl-0 hover:bg-transparent hover:text-foreground"
        href="/"
    >
        ...
        <Card.Header class="pb-4">
            <div class="flex items-start justify-between">
                <div>
                    <div class="flex items-center gap-3">
                        <Card.Title class="text-xl"
                            >Report #{data.report.id}</Card.Title
                        >
                        <span
                            class="inline-flex rounded-full border px-2.5 py-0.5 text-xs font-medium {statusColors[
                                data.report.status
                            ]}"
                        >
                ...
                <div class="flex items-center gap-2">
                        ...
                        <div class="flex items-center gap-1.5 mr-2">
                            ...
                            {#each otherViewers as viewer}
                                <Tooltip.Root>
                                    <Tooltip.Trigger>
                                        <div
                                            class="relative flex h-6 w-6 items-center justify-center rounded-full bg-green-500/20 text-xs font-medium text-green-400 border border-green-500/30"
                                        >
                                            {(
                                                viewer.displayname ||
                                                viewer.username
                                                ...
                                                .toUpperCase()}
                                            <span
                                                class="absolute -bottom-0.5 -right-0.5 h-2 w-2 rounded-full bg-green-500 border border-background"
                                            ></span>
                                        </div>
                                    </Tooltip.Trigger>
                                    <Tooltip.Content>
                                        {viewer.displayname || viewer.username} is
                                        viewing this report
                                    </Tooltip.Content>
                                </Tooltip.Root>
                            {/each}
                    ...
                    >
                        <Select.Trigger
                            class="w-35 disabled:cursor-not-allowed disabled:opacity-50"
                            disabled={statusUpdating}
                        >
                            {statusLabels[data.report.status]}
                        </Select.Trigger>
    ...
    <Card.Root>
        <Card.Header>
            <Card.Title class="text-lg"
                >Attached Files ({data.files.length})</Card.Title
            >
        </Card.Header>
        <Card.Content>
            {#if data.files.length > 0}
                <!-- Screenshot previews -->
                {@const screenshots = data.files.filter(
                    (f) => f.file_role === "screenshot",
                )}
                {#if screenshots.length > 0}
                    <div class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
                        {#each screenshots as file}
                            ...
                            >
                                <img
                                    src="/api/reports/{data.report
                                        .id}/files/{file.id}"
                                    alt={file.filename}
                                    class="w-full"
                                />
                                <div
                                    class="bg-muted/50 px-3 py-2 text-xs text-muted-foreground"
                                >
                                    {file.filename}
                                </div>
                            </div>
                    ...
                    <Table.Root>
                        ...
                        <Table.Body>
                            {#each data.files as file}
                                {@const Icon =
                                    roleIcons[file.file_role] || FileText}
                                <Table.Row>
                                    <Table.Cell>
                                        <span
                                        ...
                                        >
                                            <Icon class="h-4 w-4" />
                                            {roleLabels[file.file_role] ||
                                                file.file_role}
                                        </span>
                                    </Table.Cell>
                                    <Table.Cell class="font-mono text-xs"
                                        >{file.filename}</Table.Cell
                                    >
    ...
    <AlertDialog.Content>
        <AlertDialog.Header>
            ...
            <AlertDialog.Description>
                Are you sure you want to delete report #{data.report.id}? This
                will permanently remove the report and all attached files. This
