...
{:else}
    <Sidebar.Provider>
        <Sidebar.Root>
            <Sidebar.Header>
                <div
                    class="flex items-center justify-center border-b border-border p-3"
                    style="padding: 12px; display: flex; justify-content: center;"
                >
                    <Bug class="h-6 w-6 text-primary" />
                    <span class="mt-2 pl-3 text-lg font-bold"
                        >EMLy Dashboard</span
                    >
                </div>
            </Sidebar.Header>
                        ...
                        >
                            {#snippet child({ props })}
                                <div {...props}>
                                    ...
                                    >
                                        <span
                                            class="pb-1 truncate font-semibold"
                                            >{data.user.displayname ||
                                                data.user.username}</span
                                        >
                                        <span
                                            class="w-fit inline-flex rounded-full border px-2 py-0.5 text-xs font-medium truncate {data
                                                .user.role === 'admin'
                ...
                <div class="flex items-center gap-2 px-4">
                        ...
                        <div class="flex items-center gap-1.5">
                            {#each otherActiveUsers.slice(0, 5) as activeUser}
                                <Tooltip.Root>
                                    <Tooltip.Trigger>
                                        <div
                                            class="relative flex h-7 w-7 items-center justify-center rounded-full bg-green-500/20 text-xs font-medium text-green-400 border border-green-500/30"
                                        >
                                            {(
                            ...
                            {#if otherActiveUsers.length > 5}
                                <span class="text-xs text-muted-foreground"
                                    >+{otherActiveUsers.length - 5}</span
                                >
                            {/if}
                        </div>
                    {/if}
                    ...
                    {#if data.newCount > 0}
                        ...
                        >
                            ...
                            ></span>
                            {data.newCount} new {data.newCount === 1
                                ? "report"
                                : "reports"}
                        </div>
                    {/if}
