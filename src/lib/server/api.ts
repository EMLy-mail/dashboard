...
function baseHeaders(sessionId?: string, dbEnv?: string): Record<string, string> {
...
async function apiFetch<T>(
  path: string,
  options: RequestInit & { sessionId?: string; dbEnv?: string } = {},
): Promise<T> {
  const { sessionId, dbEnv, ...fetchOptions } = options;

  const res = await fetch(`${SERVER_URL}${path}`, {
    ...fetchOptions,
    headers: {
      ...baseHeaders(sessionId, dbEnv),
...
export async function countNewReports(dbEnv?: string): Promise<number> {
  const res = await apiFetch<{ count: number }>("/api/admin/bug-reports/count", { dbEnv });
  return res.count;
}
...
export async function deleteReport(id: number, dbEnv?: string): Promise<void> {
  await apiFetch(`/api/admin/bug-reports/${id}`, { method: "DELETE", dbEnv });
}

export async function downloadZip(id: number, dbEnv?: string): Promise<Response> {
  const headers: Record<string, string> = {
    "X-Admin-Key": ADMIN_KEY,
    "X-Api-Key": API_KEY,
  };
  ...
  const res = await fetch(
    `${SERVER_URL}/api/admin/bug-reports/${id}/download`,
    { headers },
  );
...
// Users
export async function listUsers(dbEnv?: string): Promise<User[]> {
  return apiFetch("/api/admin/users", { dbEnv });
}
...
export async function deleteUser(id: string, dbEnv?: string): Promise<void> {
  try {
    await apiFetch(`/api/admin/users/${id}`, {
      method: "DELETE",
      dbEnv,
